...
interface State {
    clickedArea: any | undefined,
    carsCoordinates: any[] 
}
interface listInterfase {
    0: number,
    1: number,
}
const createCarsList = (resp: any[]) => {
    let list: listInterfase[] = []
...
const rawCreator = (coords: number[]) => {
    return({
...
const goodAreaClicked = (coords: number[]) => {
...
class PREMap extends React.Component<Props, State>{
    ...
    clickEvent(e: any) {
        ...
        if (goodAreaClicked(coords)) {
            let raw = rawCreator(coords)
